...
-- 12.1
-- Cerin??: S? se ob?in? �nso?itorii de zbor al c?ror nume con?ine litera �a� care au participat la zborurile din anii 2020 ?i 2021
-- care au plecat din ora?ul Frankfurt  ?i care au avut gradul de ocupare (c�?i pasageri au trecut de check-in
-- fa?? de c�te locuri sunt �n avionul respectiv) mai mic de 40%. 
...
-- Elemente: subcerere nesincronizat? �n clauza from, ordon?ri ?i utilizarea func?iilor nvl ?i decode 
...
-- care au �ncasat cei mai pu?ini bani din v�nzarea cu plat? de tip online a locurilor la clasa business, 
-- dar care au �ncasat mai mult de 100 din v�nzarea acestor bilete (dac? sunt mai multe zboruri cu aceea?i sum? de bani minim?, se afi?eaz? toate). 
-- Elemente: subcerere sincronizat? �n care intervin cel pu?in 3 tabele, grup?ri de date cu subcereri nesincronizate �n care intervin cel pu?in 3 tabele, 
...
-- 12.4
-- Cerin??: Se cere ca pentru fiecare rezervare care a fost f?cut? cu cel pu?in o lun? ?i jum?tate �nainte de data de plecare a zborului s? se afle
-- dac? rezervarea a fost f?cut? de c?tre pasager (acest lucru se poate afla compar�nd numar_id, adic? atributul de cnp/security number), 
...
-- de c?tre o agen?ie de turism sau de c?tre un reprezentant de v�nz?ri. 
...
-- 12.5
-- Cerin??: S? se ob?in? zborurile care au num?rul de rezerv?ri pentru fiecare clas? (cele care au fost disponibile �n acel zbor)
-- mai mare dec�t media num?rului de rezerv?ri pentru clasa respectiv?. 
-- Elemente: subcerere nesincronizat? �n clauza from, grup?ri de date cu subcereri nesincronizate in care intervin cel pu?in 3 tabele, 
...
-- la care au fost rezervate mai pu?in de jum?tate din locurile puse la v�nzare la clasa economy.                     
commit;
...
-- 13.2: Actualizare
-- Cerin??: S? se schimbe tipul de plat? din online �n transfer bancar pentru toate pl??ile f?cute pentru rezerv?ri 
...
-- 13.3: Suprimare
-- Cerin??: S? se ?tearg? toate ora?ele �n care nu ajunge niciun zbor. 
commit;
...
-- 14.2: Division
-- Cerin??: S? se afi?eze id-ul ?i numele pasagerilor care au avut rezerv?ri �n zboruri care au ajuns 
-- �n toate aeroporturile �n care au ajuns zborurile �n care pasagerul cu id-ul 11 a avut rezervare. 
